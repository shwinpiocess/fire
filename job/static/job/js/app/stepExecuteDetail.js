    ...
    var language = {
              ...
              success: function(reponseText){
                  if(reponseText.success){
                      ...
                      if(data){
                         ...
                         //步骤详情                          
                         if(!data.isFinished){
                        	 timer = window.setTimeout(getStepExecuteDetail(stepInstanceId,retryCount,newMd5,type,name,operator), 500); 
                         } 
                      }else{
                    	  timer = window.setTimeout(getStepExecuteDetail(stepInstanceId,retryCount,newMd5,type,name,operator), 500);
                      }
     ...
     function getStepDetail(data){
    	 if(data.type == 1){
        	 ...
        	 var codeMir = $('#codeDiv').codeMirror({readOnly:true});
        	 ...
        	 data.fileSource.forEach(function(v,i){
        		 ...
        		 if(!v.account){
        			 fileList = (v.file.indexOf('/') != -1 ?
                                    v.file.slice(v.file.lastIndexOf('/')+1) : v.file.indexOf('\\') != -1 ?
                                    v.file.slice(v.file.lastIndexOf('\\')+1) : v.file);
        			 serverAddress ='本地文件';
        			 account = "无";
        		 }else{
        			 fileList = v.file;
 	 			...
 	 			temp+='<td style="width: 15%;" class="issue-info"><span>'+account+'</span></td></tr>';
        	 }) 
